...
const NAV_ITEMS = [
  ...
  { href: "/breakdowns", label: "Breakdowns", icon: Wrench, notifyKey: "breakdowns" as const },
  ...
  { href: "/announcements", label: "Announcements", icon: Megaphone, notifyKey: "announcements" as const },
    ...
    <div className="flex h-full flex-col bg-white">
      ...
      <nav className="flex-1 space-y-0.5 overflow-y-auto p-3">
        {NAV_ITEMS.map((item) => {
          ...
          const unread = item.notifyKey ? unreadCounts[item.notifyKey] : 0;
          return (
            <Link
              key={item.href}
              href={item.href}
